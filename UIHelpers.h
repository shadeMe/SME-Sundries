...
#include "WindowEdgeSnapper.inl"
		...
		// pilfered from the winspy++ source code
		namespace WinSpy
		{
			...
			//
			inline BOOL CALLBACK FindBestChildProc(HWND hwnd, LPARAM lParam)
			{
				RECT  rect;
				DWORD a;
				POINT pt;
			...
			//
			inline HWND FindBestChild(HWND hwndFound, POINT pt)
			{
				HWND  hwnd;
				DWORD dwStyle;
			...
			//
			inline HWND WindowFromPointEx(POINT pt, BOOL fShowHidden)
			{
				HWND hWndPoint;
